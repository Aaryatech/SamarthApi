...
//	}
	...
	public @ResponseBody ErrorMessage generateBill(@RequestParam("userId") int userId,
			@RequestParam("discount") float discount, @RequestParam("tableNo") int tableNo,
		...
		try {
			...
			if (orderDetails.size() > 0) {
				...
				/*
				 * for(int i = 0; i<orderDetails.size(); i++) { for(int j = 0; j<itemList.size()
				 * ; j++) { if(orderDetails.get(i).getItemId() == itemList.get(j).getItemId()) {
				 ...
				 * billDetails.setTotal(orderDetails.get(i).getTotal()); //float tax =
				 * itemList.get(j).getCgst() + itemList.get(j).getSgst();
				 * billDetails.setTaxableAmt(billDetails.getTotal()-((discount/100)*billDetails.
				 * getTotal())); float cgstAmt =
				 * (itemList.get(j).getCgst()/100)*billDetails.getTaxableAmt(); float sgstAmt =
				 * (itemList.get(j).getSgst()/100)*billDetails.getTaxableAmt();
				 * 
				 * billDetails.setTotalTax(cgstAmt+sgstAmt); billDetails.setSgst(cgstAmt);
				 * billDetails.setCgst(sgstAmt); billDetails.setDelStatus(1);
				 * 
				 * grandTotal = grandTotal+billDetails.getTotal(); cgst =
				 * cgst+billDetails.getCgst(); sgst = sgst+billDetails.getSgst(); taxableAmt =
				 * taxableAmt+billDetails.getTaxableAmt(); finalTaxAmt = finalTaxAmt +
				 * billDetails.getTotalTax();
				 * 
				 * billDetailsList.add(billDetails); break; } } }
				 */
				...
				for (int i = 0; i < orderDetails.size(); i++) {
					for (int j = 0; j < itemList.size(); j++) {
						if (orderDetails.get(i).getItemId() == itemList.get(j).getItemId()) {
							...
							float baseRate = ((billDetails.getRate() * 100)
									/ ((itemList.get(j).getCgst() + itemList.get(j).getSgst()) + 100));
				...
				if (finalsave != null && saveDetail != null) {
					...
					errorMessage.setMessage(save.getBillNo() + "#" + save.getBillId());
	...
	public @ResponseBody ErrorMessage generateBillForParcelOrder(@RequestParam("userId") int userId,
			@RequestParam("discount") float discount, @RequestParam("parcelOrderId") int parcelOrderId,
			@RequestParam("venueId") int venueId) {
		...
		try {
			...
			if (parcelOrderDetails.size() > 0) {
				...
				/*
				 * for(int i = 0; i<parcelOrderDetails.size(); i++) { for(int j = 0;
				 * j<itemList.size() ; j++) { if(parcelOrderDetails.get(i).getItemId() ==
				 * itemList.get(j).getItemId()) { BillDetails billDetails = new BillDetails();
				 * billDetails.setBillId(save.getBillId());
					...
					for (int j = 0; j < itemList.size(); j++) {
						if (parcelOrderDetails.get(i).getItemId() == itemList.get(j).getItemId()) {
							...
							grandTotal = grandTotal
									+ (parcelOrderDetails.get(i).getRate() * parcelOrderDetails.get(i).getQuantity());
							cgst = cgst + billDetails.getCgst();
							sgst = sgst + billDetails.getSgst();
							taxableAmt = taxableAmt + billDetails.getTaxableAmt();
							finalTaxAmt = finalTaxAmt + billDetails.getTotalTax();
	...
	public @ResponseBody List<ItemWithOffer> getItemListWithOfferScheme(@RequestParam("status") String status) {
		 ...
		 * the week abbreviated System.out.println(simpleDateformat.format(now));
		 * 
		 * simpleDateformat = new SimpleDateFormat("EEEE"); // the day of the week
		 * spelled out completely System.out.println(simpleDateformat.format(now));
		 */
		...
		try {
			...
			/*
			 * for(int i = 0 ; i<dateWiseList.size();i++) { for(int j = 0 ;
			 * j<dayWiseList.size();j++) {
			 * if(dateWiseList.get(i).getItemId()==dayWiseList.get(j).getItemId() &&
			 * dayWiseList.get(j).getOfferId()!=0 &&
			 ...
			 * dateWiseList.get(i).setOfferDatewiseDaywise(dayWiseList.get(j).
			 * getOfferDatewiseDaywise()); break; } } }
			 */

			for (int i = 0; i < dateWiseList.size(); i++) {
				for (int j = 0; j < dayWiseList.size(); j++) {
					if (dateWiseList.get(i).getItemId() == dayWiseList.get(j).getItemId()
							&& dateWiseList.get(i).getOfferId() == 0
							&& dayWiseList.get(j).getOfferId() > dateWiseList.get(i).getOfferId()) {
						dateWiseList.get(i).setOfferId(dayWiseList.get(j).getOfferId());
						dateWiseList.get(i).setOfferName(dayWiseList.get(j).getOfferName());
						dateWiseList.get(i).setOfferDesc(dayWiseList.get(j).getOfferDesc());
	...
	public @ResponseBody Bill editBill(@RequestBody Bill bill) {
		...
		try {
			...
			for (int i = 0; i < bill.getBillDetails().size(); i++) {
				for (int j = 0; j < itemList.size(); j++) {
					if (bill.getBillDetails().get(i).getItemId() == itemList.get(j).getItemId()
							&& bill.getBillDetails().get(i).getDelStatus() == 1) {

						float baseRate = ((bill.getBillDetails().get(i).getRate() * 100)
								/ ((itemList.get(j).getCgst() + itemList.get(j).getSgst()) + 100));
						...
						float cgstAmt = (itemList.get(j).getCgst() / 100)
								* bill.getBillDetails().get(i).getTaxableAmt();
						float sgstAmt = (itemList.get(j).getSgst() / 100)
								* bill.getBillDetails().get(i).getTaxableAmt();
						...
						bill.getBillDetails().get(i)
								.setTotal(bill.getBillDetails().get(i).getTaxableAmt() + cgstAmt + sgstAmt);

						grandTotal = grandTotal
								+ (bill.getBillDetails().get(i).getRate() * bill.getBillDetails().get(i).getQuantity());
	...
	public @ResponseBody List<GetBill> getBillHeaderAndDetail(@RequestParam("date") String date,
			@RequestParam("type") int type, @RequestParam("bill_closed") int bill_closed) {
		...
		try {
			...
			for (int i = 0; i < getBillHeaderAndDetail.size(); i++) {
				List<BillDetails> getDetail = billDetailsRepository
						.getDetail(getBillHeaderAndDetail.get(i).getBillId());
	...
	// 27-11-2019-----
	@RequestMapping(value = { "/getTaxableDataForBillPrintByBillId" }, method = RequestMethod.POST)
	public @ResponseBody List<TaxableDataForBill> getTaxableDataForBillPrintByBillId(
			@RequestParam("billIds") List<Integer> billIds) {

		List<TaxableDataForBill> resultList = new ArrayList<TaxableDataForBill>();
		try {

			resultList = taxableDataForBillRepo.getTaxableDataByBillId(billIds);
	...
	// 27-11-2019-----
	@RequestMapping(value = { "/getTaxableDataForBillPrintByDate" }, method = RequestMethod.POST)
	public @ResponseBody List<TaxableDataForBill> getTaxableDataForBillPrintByDate(
			@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate) {

		List<TaxableDataForBill> resultList = new ArrayList<TaxableDataForBill>();
		try {

			resultList = taxableDataForBillRepo.getTaxableDataByDate(fromDate, toDate);
